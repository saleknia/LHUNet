...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda'), args: dict = None):
        ...
        self.print_to_log_file("\n#######################################################################\n"
                               "Please cite the following papers when using nnU-Net and LHU-Net:\n"
                               "Sadegheih, Y., Bozorgpour, A., Kumari, P., Azad, R., & Merhof, D. (2025)."
                               "LHU-Net: A lean hybrid u-net for cost-efficient, high-performance volumetric medical image segmentation."
                               "International Conference on Medical Image Computing and Computer-Assisted Intervention. Cham: Springer Nature Switzerland.\n\n\n"
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            ############## Yousef Modifications for the calculation of FLOPs and Parameters ##############
            n_parameters = sum(
            ...
            self.print_to_log_file(
                f"Total trainable parameters: {round(n_parameters * 1e-6, 3)} M"
            )
            self.print_to_log_file(f"MAdds: {round(model_flops * 1e-9, 3)} G")
    ...
    def print_args(self):
        self.print_to_log_file("*" * 10)
        self.print_to_log_file("Arguments:")
        for arg in vars(self.args):
            self.print_to_log_file(f"{arg}: {getattr(self.args, arg)}")
        self.print_to_log_file("*" * 10)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if (self.device.type == 'cuda' and self.args.amp) else dummy_context():
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.identifiers):
                ...
                ########### YOUSEF HERE ###########
                prediction = predictor.predict_sliding_window_return_logits(data, self.args.amp)
